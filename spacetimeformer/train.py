...
def create_model(config):
    ...
    if config.dset == "metr-la":
        ...
        # yc_dim = 137
        # yt_dim = 137
        yc_dim = 9 # edit here -- num column of dataset
        yt_dim = 9
        ...
        yc_dim = 44
        yt_dim = 44
    elif config.dset == "agri":
        x_dim = 3
        yc_dim = 4
        yt_dim = 4  # or e.g., yt_dim = 5 (open high low close volume)
...
def create_dset(config):
    ...
    if config.dset == "metr-la" or config.dset == "pems-bay":
        ...
        if config.dset == "asos":
            ...
            target_cols = [
                        "ETH_open", "ETH_high", "ETHT_low", "ETH_close", "Volume ETH", "Volume USDT", "ETH_tradecount",
                        "BTC_open", "BTC_high", "BTC_low", "BTC_close", "Volume BTC", "BTC_tradecount",
                        "LINK_open", "LINK_high", "LINK_low", "LINK_close", "Volume LINK", "LINK_tradecount",
                        "EOS_open", "EOS_high", "EOS_low", "EOS_close", "Volume EOS", "EOS_tradecount",
                        "XMR_open", "XMR_high", "XMR_low", "XMR_close", "Volume XMR", "XMR_tradecount",
                        "NEO_open", "NEO_high", "NEO_low", "NEO_close", "Volume NEO", "NEO_tradecount",
                        "LTCUSDT_open", "LTCUSDT_high", "LTCUSDT_low", "LTCUSDT_close", "Volume LTC", "LTCUSDT_tradecount",
                        "sntiments"
                    ]
            # target_cols = [
            #     "ETH_close",
            ...
            if data_path == "auto":
                data_path = "./data/agri_dset.csv"
            target_cols = ["price_RICE", "price_CORN", "price_CASSAVA", "price_SUGARCRANE"]
            time_col_name = "date"
            time_format = "%Y-%m-%d"
            time_features = ["month", "day", "weekday"]
        elif config.dset == "solar_energy":
            if data_path == "auto":
                data_path = "./data/solar_AL_converted.csv"
            target_cols = [str(i) for i in range(9)]
